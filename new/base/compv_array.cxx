...
template<class T>
CompVArray<T>::CompVArray(COMPV_ARRAY_TYPE type /*= COMPV_ARRAY_TYPE_RAW*/, COMPV_ARRAY_SUBTYPE subType /*= COMPV_ARRAY_SUBTYPE_RAW*/)
	: m_eType(type)
	, m_eSubType(subType)
	, m_pDataPtr(NULL)
	, m_bOweMem(true)
...
template<class T>
COMPV_ERROR_CODE CompVArray<T>::newObj(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, size_t alignv, COMPV_ARRAY_TYPE type /*= COMPV_ARRAY_TYPE_RAW*/, COMPV_ARRAY_SUBTYPE subType /*= COMPV_ARRAY_SUBTYPE_RAW*/, size_t stride /*= 0*/)
{
    COMPV_CHECK_EXP_RETURN(!array || !alignv, COMPV_ERROR_CODE_E_INVALID_PARAMETER);
	CompVPtr<CompVArray<T>* > array_ = *array;
	if (!array_) {
		array_ = new CompVArray<T>(type, subType);
...
template<class T>
COMPV_ERROR_CODE CompVArray<T>::newObjStrideless(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, COMPV_ARRAY_TYPE type /*= COMPV_ARRAY_TYPE_RAW*/, COMPV_ARRAY_SUBTYPE subType /*= COMPV_ARRAY_SUBTYPE_RAW*/)
{
	return CompVArray<T>::newObj(array, rows, cols, 1, type, subType);
}

template<class T>
COMPV_ERROR_CODE CompVArray<T>::newObjAligned(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, COMPV_ARRAY_TYPE type /*= COMPV_ARRAY_TYPE_RAW*/, COMPV_ARRAY_SUBTYPE subType /*= COMPV_ARRAY_SUBTYPE_RAW*/)
{
	return CompVArray<T>::newObj(array, rows, cols, COMPV_SIMD_ALIGNV_DEFAULT, type, subType);
}
