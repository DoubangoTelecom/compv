...
*/
#if !defined(_COMPV_BASE_ARRAY_H_)
#define _COMPV_BASE_ARRAY_H_
...
template<class T>
class COMPV_BASE_API CompVArray : public CompVObj
{
protected:
	CompVArray(COMPV_ARRAY_TYPE type = COMPV_ARRAY_TYPE_RAW, COMPV_ARRAY_SUBTYPE subType = COMPV_ARRAY_SUBTYPE_RAW);
public:
    virtual ~CompVArray();
	...
	COMPV_INLINE COMPV_ARRAY_TYPE type()const {
		return m_eType;
	}
	COMPV_INLINE COMPV_ARRAY_SUBTYPE subType()const { // can be casted to 'COMPV_PIXEL_FORMAT'
		return m_eSubType;
	}
	...
	COMPV_INLINE COMPV_ERROR_CODE strideInElts(size_t &strideInElts)const {
	...
	static COMPV_ERROR_CODE newObj(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, size_t alignv, COMPV_ARRAY_TYPE type = COMPV_ARRAY_TYPE_RAW, COMPV_ARRAY_SUBTYPE subType = COMPV_ARRAY_SUBTYPE_RAW, size_t stride = 0);
	static COMPV_ERROR_CODE newObjStrideless(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, COMPV_ARRAY_TYPE type = COMPV_ARRAY_TYPE_RAW, COMPV_ARRAY_SUBTYPE subType = COMPV_ARRAY_SUBTYPE_RAW);
	static COMPV_ERROR_CODE newObjAligned(CompVPtr<CompVArray<T>* >* array, size_t rows, size_t cols, COMPV_ARRAY_TYPE type = COMPV_ARRAY_TYPE_RAW, COMPV_ARRAY_SUBTYPE subType = COMPV_ARRAY_SUBTYPE_RAW);
...
private:
	...
	COMPV_ARRAY_TYPE m_eType;
	COMPV_ARRAY_SUBTYPE m_eSubType;
    COMPV_DISABLE_WARNINGS_END()
};

COMPV_NAMESPACE_END()

#endif /* _COMPV_BASE_ARRAY_H_ */
