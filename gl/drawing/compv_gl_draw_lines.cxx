...
COMPV_ERROR_CODE CompVGLDrawLines::lines(const CompVGLPoint2D* lines, const GLsizei count, const CompVDrawingOptions* options COMPV_DEFAULT(nullptr))
{
	...
	const COMPV_DRAWING_LINE_CONNECT lineConnect = options ? options->lineConnect : COMPV_DRAWING_LINE_CONNECT_NONE;
	...
	COMPV_glDrawArrays(
		(lineConnect == COMPV_DRAWING_LINE_CONNECT_LOOP) ? GL_LINE_LOOP : (lineConnect == COMPV_DRAWING_LINE_CONNECT_STRIP ? GL_LINE_STRIP : GL_LINES),
		0, count
	);
