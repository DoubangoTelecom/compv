...
static const std::string compv_unittest_hogs_to_string(const compv_unittest_hogstd* test) {
	return std::string("filename:") 
		...
		+ std::string(", norm:") + compv_unittest_norm_to_string(test->norm)
		+ std::string(", inter:") + std::string(test->interp == COMPV_HOG_INTERPOLATION_BILINEAR ? "Bilinear" : "BilinearLUT");
}
...
COMPV_ERROR_CODE unittest_hog_s()
{
	...
	for (size_t i = 0; i < COMPV_UNITTEST_HOGSTD_COUNT; ++i) {
		...
		COMPV_CHECK_CODE_RETURN(hogStd->setInt(COMPV_HOG_SET_INT_INTERPOLATION, test->interp));
