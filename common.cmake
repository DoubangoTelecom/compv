...
macro(set_INTRIN_COMPILE_FLAGS)
	foreach (file ${ARGN})
		...
		if (${file_name} MATCHES "_intrin_(.*_)?sse2.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_SSE2}")
		elseif (${file_name} MATCHES "_intrin_(.*_)?ssse3.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_SSSE3}")
		elseif (${file_name} MATCHES "_intrin_(.*_)?sse41.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_SSE41}")
		elseif (${file_name} MATCHES "_intrin_(.*_)?sse42.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_SSE42}")
		elseif (${file_name} MATCHES "_intrin_(.*_)?avx.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_AVX}")
		elseif (${file_name} MATCHES "_intrin_(.*_)?avx2.cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_AVX2}")
		endif ()
		# Append others
		if (${file_name} MATCHES "_intrin_fma(3|4)_(.*).cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_FMA}")
		endif ()
		if (${file_name} MATCHES "_intrin_bmi1_(.*).cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_BMI1}")
		endif ()
		if (${file_name} MATCHES "_intrin_bmi2_(.*).cxx")
			set_source_files_properties(${file} APPEND PROPERTY COMPILE_FLAGS "${FLAGS_BMI2}")
		endif ()
	endforeach()
endmacro()
