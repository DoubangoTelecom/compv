...
static void scaleBilinearKernel11_C(const uint8_t* inPtr, uint8_t* outPtr, vcomp_scalar_t inHeight, vcomp_scalar_t inWidth, vcomp_scalar_t inStride, vcomp_scalar_t outHeight, vcomp_scalar_t outWidth, vcomp_scalar_t outStride, vcomp_scalar_t sf_x, vcomp_scalar_t sf_y)
{
...
COMPV_ERROR_CODE CompVImageScaleBilinear::process(const CompVObjWrapper<CompVImage* >& inImage, CompVObjWrapper<CompVImage* >& outImage)
{
	...
	vcomp_scalar_t inWidths[4], outWidths[4];
	vcomp_scalar_t inHeights[4], outHeights[4];
	vcomp_scalar_t inStrides[4], outStrides[4];
	int compSize = 1;
	...
	for (int k = 0; k < compSize; ++k) {
		...
		const vcomp_scalar_t int_sx = (vcomp_scalar_t)(float_sx * 256.f); // do not use "<< 8" to include the error
		const vcomp_scalar_t int_sy = (vcomp_scalar_t)(float_sy * 256.f);  // do not use "<< 8" to include the error
