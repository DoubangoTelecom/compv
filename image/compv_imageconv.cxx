...
static COMPV_ERROR_CODE __rgbaToI420(const uint8_t* rgbaPtr, int32_t height, int32_t width, int32_t stride, CompVObjWrapper<CompVImage* >* i420, int compSize, int ridx, int gidx, int bidx, int aidx = INT_MAX)
{
	...
	switch (compSize) {
	case 4:
		if (aidx == 0 && ridx == 1) COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER); // argb
		else if (aidx == 0 && bidx == 1) COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER); // abgr
		else if (aidx == 3 && ridx == 0) COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER); // rgba
		else if (aidx == 3 && bidx == 1) COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER); // bgra
		else COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER);
		break;
	case 3:
		if (ridx == 0) ; //rgb
		else if (ridx == 2) COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER); //bgr
		else COMPV_CHECK_CODE_RETURN(COMPV_ERROR_CODE_E_INVALID_PARAMETER);
		break;
	default:
