...
COMPV_ERROR_CODE CompVCornerDeteEdgeBase::process(const CompVMatPtr& image, CompVMatPtrPtr edges, CompVMatPtrPtr directions COMPV_DEFAULT(NULL)) /*Overrides(CompVEdgeDete)*/
{
	...
	if (threadsCount > 1) {
		...
		auto funcConvolutionAndGradient = [&](const uint8_t* ptrIn, int16_t* ptrOutGx, int16_t* ptrOutGy, uint16_t* ptrOutG, compv_float32_t* ptrGradDir, const size_t h, const size_t threadIdx) -> COMPV_ERROR_CODE {
			...
			COMPV_CHECK_CODE_RETURN((CompVMathConvlt::convlt1Hz<uint8_t, int16_t, int16_t>(ptrIn - padding, imgTmp, m_nImageWidth, h + rowsOverlapCount, m_nImageStride, m_pcKernelHz, m_nKernelSize, COMPV_BORDER_TYPE_ZERO)));
			COMPV_CHECK_CODE_RETURN((CompVMathConvlt::convlt1Vt<int16_t, int16_t, int16_t>(imgTmp, ptrOutGx - padding, m_nImageWidth, h + rowsOverlapCount, m_nImageStride, m_pcKernelVt, m_nKernelSize, first ? COMPV_BORDER_TYPE_ZERO : COMPV_BORDER_TYPE_IGNORE, last ? COMPV_BORDER_TYPE_ZERO : COMPV_BORDER_TYPE_IGNORE)));
			COMPV_CHECK_CODE_RETURN((CompVMathConvlt::convlt1Hz<uint8_t, int16_t, int16_t>(ptrIn - padding, imgTmp, m_nImageWidth, h + rowsOverlapCount, m_nImageStride, m_pcKernelVt, m_nKernelSize, COMPV_BORDER_TYPE_ZERO)));
			COMPV_CHECK_CODE_RETURN((CompVMathConvlt::convlt1Vt<int16_t, int16_t, int16_t>(imgTmp, ptrOutGy - padding, m_nImageWidth, h + rowsOverlapCount, m_nImageStride, m_pcKernelHz, m_nKernelSize, first ? COMPV_BORDER_TYPE_ZERO : COMPV_BORDER_TYPE_IGNORE, last ? COMPV_BORDER_TYPE_ZERO : COMPV_BORDER_TYPE_IGNORE)));
