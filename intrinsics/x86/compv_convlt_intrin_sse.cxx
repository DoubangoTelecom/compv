...
// FIXME: remove
void Convlt1_vert_float32_minpack4_Intrin_SSE2(const uint8_t* in_ptr, uint8_t* out_ptr, compv_scalar_t width, compv_scalar_t height, compv_scalar_t stride, compv_scalar_t pad, const float* vkern_ptr, compv_scalar_t kern_size)
{
	COMPV_DEBUG_INFO_CODE_NOT_OPTIMIZED(); // ASM

#if 1
	compv_scalar_t i, j, row;
	__m128 xmmCoeff, xmmF0, xmmSF0, xmmSF1, xmmSF2, xmmSF3;
	__m128i xmmI0, xmmI1, xmmI2, xmmI3, xmmZero;
	...
	for (j = 0; j < height; ++j) {
		...
		while (i > 15) {
			...
			for (row = 0; row < kern_size; ++row) {
				xmmI0 = _mm_loadu_si128((__m128i*)&in_ptr[row * stride]);
				xmmCoeff = _mm_set1_ps(vkern_ptr[row]);

				// TODO(dmi): Very good candidate for FMA3 / AVX (done for ASM)

				xmmI1 = _mm_unpacklo_epi8(xmmI0, xmmZero);

				xmmF0 = _mm_cvtepi32_ps(_mm_unpacklo_epi16(xmmI1, xmmZero));
				xmmF0 = _mm_mul_ps(xmmF0, xmmCoeff);
				xmmSF0 = _mm_add_ps(xmmSF0, xmmF0);

				xmmF0 = _mm_cvtepi32_ps(_mm_unpackhi_epi16(xmmI1, xmmZero));
				xmmF0 = _mm_mul_ps(xmmF0, xmmCoeff);
				xmmSF1 = _mm_add_ps(xmmSF1, xmmF0);

				xmmI1 = _mm_unpackhi_epi8(xmmI0, xmmZero);
		...
		while (i > 3) {
			xmmSF0 = _mm_setzero_ps();
			for (row = 0; row < kern_size; ++row) {
				xmmI0 = _mm_cvtsi32_si128(*((uint32_t*)&in_ptr[row * stride]));
				xmmCoeff = _mm_set1_ps(vkern_ptr[row]);
