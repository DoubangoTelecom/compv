...
#define TAG_SAMPLE			"Adaptive Thresholding"
...
{
	{
		...
		// Create mean kernel
		COMPV_CHECK_CODE_BAIL(err = CompVImageThreshold::kernelMean(BLOCK_SIZE, &kernel));
		...
		COMPV_CHECK_CODE_BAIL(err = camera->setCallbackOnNewFrame([&](const CompVMatPtr& image) -> COMPV_ERROR_CODE {
			...
			if (CompVDrawing::isLoopRunning()) {
				...
				COMPV_CHECK_CODE_RETURN(CompVImageThreshold::adaptive(imageGray, &imageGray, kernel, DELTA, MAXVAL, INVERT));
				
				COMPV_CHECK_CODE_BAIL(err = window->beginDraw());
				COMPV_CHECK_CODE_BAIL(err = singleSurfaceLayer->cover()->drawImage(imageGray));
