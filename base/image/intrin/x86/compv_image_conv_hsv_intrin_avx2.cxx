	...
	, const compv_float32_t(*scales43)[256], const compv_float32_t(*scales255)[256])
{
	...
	__m128i vec0n, vec1n, vec2n, vec3n, vec4n;
	...
	for (j = 0; j < height; ++j) {
		for (i = 0; i < width; i += 96) { // 96 = (32 * 3)
			// R = vec0, G = vec1, B = vec2, vec3 and vec4 used as temp variables
			COMPV_VLD3_U8_SSSE3(&rgb24Ptr[i], vec0n, vec1n, vec2n, vec3n, vec4n);
			...
			COMPV_VLD3_U8_SSSE3(&rgb24Ptr[i + 48], vec0n, vec1n, vec2n, vec3n, vec4n);
