...
// Copyright (c) 2018 Mamadou DIOP (Doubango Telecom) - SSE, AVX2 and NEON optimizations
//
...
// requiring Annoy to import their headers. See RcppAnnoy for a use case.
  ...
  #define showUpdate(FMT,...) { COMPV_DEBUG_VERBOSE_EX("ANNOY", FMT, ##__VA_ARGS__ ); }
#else
  #define showUpdate(FMT,...) { __ERROR_PRINTER_OVERRIDE__( __VA_ARGS__ ); }
#endif
...
inline void two_means(const vector<Node*>& nodes, int f, Random& random, bool cosine, Node* p, Node* q) {
  ...
  */
  ...
  for (int l = 0; l < iteration_steps; l++) {
    ...
    T norm = static_cast<T>(cosine ? get_norm(nodes[k]->v, f) : 1.0);
    if (!(norm > T(0))) {
      continue;
    }
    if (di < dj) {
...
struct Angular {
  ...
  static inline T distance(const Node<S, T>* x, const Node<S, T>* y, int f) {
    ...
    T pq = dot(x->v, y->v, f);
    T ppqq = pp * qq;
    if (ppqq > 0) return static_cast<T>(2.0 - 2.0 * pq / sqrt(ppqq));
...
public:
  ...
  void build(int q) {
    ...
    if (_verbose) showUpdate("has %zd nodes\n", _n_nodes);
  }
  ...
  void unload() {
    if (_fd) {
      ...
      close(_fd);
      off_t size = static_cast<off_t>(_n_nodes * _s);
      munmap(_nodes, size);
    } else if (_nodes) {
  ...
  S _make_tree(const vector<S >& indices, bool is_root) {
    ...
    while (children_indices[0].size() == 0 || children_indices[1].size() == 0) {
      if (_verbose && indices.size() > 100000)
        showUpdate("Failed splitting %zu items\n", indices.size());
