...
#include "compv/base/tbbmalloc/tbb_version.h"
#include "compv/base/tbbmalloc/itt_notify.h" // for __TBB_load_ittnotify()
...
{
    return bin + getIndex(static_cast<unsigned int>(size));
}
...
/* The debug version verifies the TLSBin as needed */
    uint32_t objSize = getObjectSize(static_cast<unsigned int>(size));
...
void Block::initEmptyBlock(TLSData *tls, size_t size)
{
    ...
    unsigned int index = getIndex(static_cast<unsigned int>(size));
    unsigned int objSz = getObjectSize(static_cast<unsigned int>(size));
...
void *MemoryPool::getFromLLOCache(TLSData* tls, size_t size, size_t alignment)
{
    ...
    if (lmb) {
        ...
        unsigned ptrDelta = static_cast<unsigned>(alignedRight - (uintptr_t)alignedArea);
        if (ptrDelta && tls) { // !tls is cold path
            ...
            unsigned numOfPossibleOffsets = static_cast<unsigned>(alignment == estimatedCacheLineSize?
...
FreeObject *Block::findAllocatedObject(const void *address) const
{
    ...
    uint16_t offset = (uint16_t)((uintptr_t)this + slabSize - (uintptr_t)address);
