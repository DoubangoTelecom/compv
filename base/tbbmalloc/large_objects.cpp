...
class CacheBinFunctor {
    ...
    public:
        ...
        uintptr_t getTimeRange() const { return -(intptr_t)lclTime; }
            ...
            do {
                ...
                if ( !isEmpty ) {
                    if ( LargeMemoryBlock *res = bin->get() ) {
                        ...
                        bin->updateCachedSize( -(intptr_t)opGetData.size );
                        *opGetData.res = res;
                    } else {
                        isEmpty = true;
    ...
    CacheBin::putList(LargeMemoryBlock *head, LargeMemoryBlock *tail, BinBitMask *bitMask, int idx, int num)
{
    ...
    usedSize -= num*size;
    MALLOC_ASSERT( !last || (last->age != 0 && last->age != -1), ASSERT_TEXT );
    ...
    CacheBin::forgetOutdatedState(uintptr_t currTime)
{
    ...
    if (ageThreshold)
        doCleanup = sinceLastGet > (uintptr_t)(Props::LongWaitFactor*ageThreshold);
...
void LargeObjectCacheImpl<Props>::updateCacheState(ExtMemoryPool *extMemPool, DecreaseOrIncrease op, size_t size)
{
    ...
    bin[idx].updateUsedSize(extMemPool, op==decrease? -(intptr_t)size : size, &bitMask, idx);
}
