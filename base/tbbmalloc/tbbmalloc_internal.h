...
#include "compv/base/tbbmalloc/tbb_config.h" // for __TBB_LIBSTDCPP_EXCEPTION_HEADERS_BROKEN
...
protected:
    ...
    int getMinTrue(unsigned startIdx) const {
        ...
        if (startIdx % WORD_LEN) {
            ...
            idx++;
            if (-1 != (pos = (int)BitScanRev(actualMask)))
                return idx*WORD_LEN - pos - 1;
        }

        while (idx<SZ)
            if (-1 != (pos = (int)BitScanRev(mask[idx++])))
                return idx*WORD_LEN - pos - 1;
        return -1;
    }
public:
...
private:
    static int sizeToBin(size_t size) {
        ...
        int bin = static_cast<int>((size - minBinnedSize)/LargeObjectCache::largeBlockCacheStep);
...
struct ExtMemoryPool {
    ...
    bool isPoolValid() const { return granularity != 0; }
};
