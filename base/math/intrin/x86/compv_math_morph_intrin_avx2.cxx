	...
	for (j = 0, k = 0; j < height; ++j) {
		for (i = 0; i < width128; i += 128, k += 128) {
			...
			for (v = 1; v < strelInputPtrsCount; ++v) {
				vec0 = _mm256_min_epu8(vec0, _mm256_loadu_si256(reinterpret_cast<const __m256i*>(k + strelInputPtrsPtr[v])));
				vec1 = _mm256_min_epu8(vec1, _mm256_loadu_si256(reinterpret_cast<const __m256i*>(k + strelInputPtrsPtr[v]) + 1));
				vec2 = _mm256_min_epu8(vec2, _mm256_loadu_si256(reinterpret_cast<const __m256i*>(k + strelInputPtrsPtr[v]) + 2));
				vec3 = _mm256_min_epu8(vec3, _mm256_loadu_si256(reinterpret_cast<const __m256i*>(k + strelInputPtrsPtr[v]) + 3));
			}
