...
void CompVMathConvlt1VtHz_8u32f32f_Intrin_AVX2(const uint8_t* inPtr, compv_float32_t* outPtr, compv_uscalar_t width, compv_uscalar_t height, compv_uscalar_t step, compv_uscalar_t pad, const compv_float32_t* vthzKernPtr, compv_uscalar_t kernSize)
{
	...
	for (j = 0; j < height; ++j) {
		for (i = 0; i < width; i += 32) {
			...
			vec0f = _mm256_permute2f128_ps(vecSum0, vecSum1, 0x20);
			vec1f = _mm256_permute2f128_ps(vecSum2, vecSum3, 0x20);
			vec2f = _mm256_permute2f128_ps(vecSum0, vecSum1, 0x31);
			vec3f = _mm256_permute2f128_ps(vecSum2, vecSum3, 0x31);
			if (i < width32) {
				_mm256_storeu_ps(&outPtr[i], vec0f);
				_mm256_storeu_ps(&outPtr[i + 8], vec1f);
				_mm256_storeu_ps(&outPtr[i + 16], vec2f);
				_mm256_storeu_ps(&outPtr[i + 24], vec3f);
			}
			else {
				_mm256_store_ps(&mem[0], vec0f);
				_mm256_store_ps(&mem[8], vec1f);
				_mm256_store_ps(&mem[16], vec2f);
				_mm256_store_ps(&mem[24], vec3f);
				for (k = 0; i < width; ++i, ++k) {
					outPtr[i] = mem[k];
				}
			}
...
void CompVMathConvlt1VtHz_8u16s16s_Intrin_AVX2(const uint8_t* inPtr, int16_t* outPtr, compv_uscalar_t width, compv_uscalar_t height, compv_uscalar_t step, compv_uscalar_t pad, const int16_t* vthzKernPtr, compv_uscalar_t kernSize)
{
	...
	const compv_uscalar_t stride = (width + pad);
	const compv_uscalar_t width32 = width & -32;
	__m256i vecInPtr, vec0, vec1, vec2, vec3, vecSum0, vecSum1, vecSum2, vecSum3, vecCoeff;
	const __m256i vecZero = _mm256_setzero_si256();
	...
	for (j = 0; j < height; ++j) {
		for (i = 0; i < width; i += 32) {
			...
			for (row = 0, k = 0; row < kernSize; ++row, k += step) {
				...
				vec2 = _mm256_unpacklo_epi8(vecInPtr, vecZero); // epu8 -> epu16
				vec3 = _mm256_unpackhi_epi8(vecInPtr, vecZero); // epu8 -> epu16
				vec0 = _mm256_unpacklo_epi16(vec2, vecZero); // epu16 -> epu32
				vec1 = _mm256_unpackhi_epi16(vec2, vecZero); // epu16 -> epu32
				vec2 = _mm256_unpacklo_epi16(vec3, vecZero); // epu16 -> epu32
				vec3 = _mm256_unpackhi_epi16(vec3, vecZero); // epu16 -> epu32
			...
			vec0 = _mm256_permute2x128_si256(vecSum0, vecSum2, 0x20);
			vec1 = _mm256_permute2x128_si256(vecSum0, vecSum2, 0x31);
			if (i < width32) {
				_mm256_storeu_si256(reinterpret_cast<__m256i*>(&outPtr[i]), vec0);
				_mm256_storeu_si256(reinterpret_cast<__m256i*>(&outPtr[i + 16]), vec1);
			}
			else {
				_mm256_storeu_si256(reinterpret_cast<__m256i*>(&mem[0]), vec0);
				_mm256_storeu_si256(reinterpret_cast<__m256i*>(&mem[16]), vec1);
				for (k = 0; i < width; ++i, ++k) {
					outPtr[i] = mem[k];
				}
			}
