...
)
{
	...
	for (compv_uscalar_t j = 0; j < height; ++j) {
		for (i = 0; i < width16; i += 16) {
			vst1q_f32(&Rptr[i], vaddq_f32(vld1q_f32(&Aptr[i]), vld1q_f32(&Bptr[i])));
			vst1q_f32(&Rptr[i + 4], vaddq_f32(vld1q_f32(&Aptr[i + 4]), vld1q_f32(&Bptr[i + 4])));
			vst1q_f32(&Rptr[i + 8], vaddq_f32(vld1q_f32(&Aptr[i + 8]), vld1q_f32(&Bptr[i + 8])));
			vst1q_f32(&Rptr[i + 12], vaddq_f32(vld1q_f32(&Aptr[i + 12]), vld1q_f32(&Bptr[i + 12])));
		}
		for (; i < width; i += 4) {
			vst1q_f32(&Rptr[i], vaddq_f32(vld1q_f32(&Aptr[i]), vld1q_f32(&Bptr[i])));
		}
