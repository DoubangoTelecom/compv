...
void CompVMathConvlt1VtHz_8u32f8u_Intrin_NEON_FMA(COMPV_ALIGNED(NEON) const uint8_t* inPtr, uint8_t* outPtr, compv_uscalar_t width, compv_uscalar_t height, compv_uscalar_t step, compv_uscalar_t pad, const compv_float32_t* vthzKernPtr, compv_uscalar_t kernSize)
{
    COMPV_DEBUG_INFO_CODE_NOT_OPTIMIZED("Very slooooow.... Function not used.");
    COMPV_DEBUG_INFO_CHECK_NEON();
    COMPV_DEBUG_INFO_CHECK_FMA();
    ...
    for (j = 0; j < height; ++j) {
        ...
        for (i = 0; i < width - 15; i += 16) {
            ...
            for (row = 0, k = 0; row < kernSize; ++row, k += step) {
                ...
                vecSum0 = COMPV_ARM_N_FMAQ(vecSum0, vec0f, coeff);
                vecSum1 = COMPV_ARM_N_FMAQ(vecSum1, vec1f, coeff);
                vecSum2 = COMPV_ARM_N_FMAQ(vecSum2, vec2f, coeff);
                vecSum3 = COMPV_ARM_N_FMAQ(vecSum3, vec3f, coeff);
            }
        ...
        for (; i < width; i += 1) {
            ...
            for (row = 0, k = 0; row < kernSize; ++row, k += step) {
                coeff = static_cast<compv_float32_t>(inPtr[i + k]);
                vecSum0n = COMPV_ARM_N_FMA(vecSum0n, vdup_n_f32(vthzKernPtr[row]), coeff);
            }
