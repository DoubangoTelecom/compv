	...
	static COMPV_ERROR_CODE line(const CompVMatPtr& points, const FloatType threshold, CompVMatPtrPtr params)
	{
		...
		if (status.numInliers >= kMinModelPoints) {
			...
			*(*params)->ptr<FloatType>(0, 2) = status.modelParamsBest[2];
			...
			// Single case could lead to this code: input data is a single and same point duplicated
			COMPV_CHECK_CODE_RETURN(CompVMat::newObjAligned<FloatType>(params, 1, kNumParamsLine));
...
private:
	...
	static COMPV_ERROR_CODE lineBuildResiduals(const CompVMathStatsRansacControl<FloatType>* control, const CompVMathStatsRansacModelParamsFloatType& modelParams, FloatType* residualPtr, bool& userbreak)
	{
